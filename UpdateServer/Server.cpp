...
int main()
{
	...
	int serverVersion = getLocalVersion();
	while (!done)
	{
		...
		{
			recvMessage[vRecv] = '\0';
			cout << "Checking incoming update of verison " << (int)recvMessage[0] << "\n";
			cout << "Latest Version is " << serverVersion << "\n\n";
			if ((int)recvMessage[0] == serverVersion)
			{   
				char noUpdate[22] = "No updates available";
				int iSend = send(acceptSocket, noUpdate, strlen(noUpdate), 0);
